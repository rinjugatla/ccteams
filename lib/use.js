...
// skill — it must survive team switches, re-applies, and package updates. The
// name is reserved: teams may not ship a skill called "team-lessons".
...
// Presence of this marker in SKILL.md means the project already uses the
// generated index. Its absence in a pre-existing SKILL.md means the project was
// scaffolded by an older ccteams and still keeps lessons inline.
const TEAM_LESSONS_CATALOG_MARKER = '<!-- team-lessons:catalog:start -->';
 ...
 *   with the real-write path on purpose — only the write calls are gated —
 *   so dry-run and real runs can never disagree about which files are missing.
 * @returns {{ created: string[], preserved: string[], needsMigration: boolean }}
 *   paths relative to destDir, plus whether a pre-existing SKILL.md still lacks
 *   the generated-index markers (i.e. the generator would refuse to run on it).
...
export function scaffoldTeamLessons(destDir, scaffoldDir = TEAM_LESSONS_SCAFFOLD_DIR, opts = {}) {
  ...
  const created = [];
  const preserved = [];
  if (!fs.existsSync(scaffoldDir)) return { created, preserved, needsMigration: false };
  ...
  const needsMigration =
    preserved.includes('SKILL.md') &&
    !fs.readFileSync(skillPath, 'utf8').includes(TEAM_LESSONS_CATALOG_MARKER);

  return { created, preserved, needsMigration };
}
