 ...
 *
 *   (a) needsMigration — the marker pair is missing (or no end marker follows the
 *       start marker). buildSkill() THROWS on exactly this condition, so
 *       re-running the generator cannot fix it; the user has to add the markers
 *       first.
...
function classifySkillLayout(content) {
  ...
  // would find that leading mention, land before the start marker, and report a
  // healthy file as needing migration.
  const endIndex =
    startIndex === -1 ? -1 : content.indexOf(CATALOG_END, startIndex + CATALOG_START.length);
  // Equivalent to the predicate buildSkill() throws on. It writes that test as
  // `endIndex < startIndex` rather than the `endIndex > startIndex` used here,
  // but both searches now start past the end of the start marker, so a found end
  // marker is always strictly further along: each form reduces to "no end marker
  // after the start marker", and the two agree on every input.
 ...
 * @param {{ dryRun?: boolean, skipRelPaths?: Iterable<string> }} [opts]
 *   dryRun: true — compute the same result
 ...
 *
 *   skipRelPaths (default: none) — forward-slash paths, relative to destDir,
 *   this call must pretend the template does not contain at all: a skipped path
 *   is neither created nor preserved, and appears in NEITHER return array, so a
 *   caller cannot accidentally report it as handled.
 *
 *   This exists for one specific correctness requirement in migrate.js (see
 *   teamLessonsTemplateStep): `scripts/template-version.mjs` is a GENERATION
 *   MARKER, and a marker on disk asserts that every ccteams-owned team-lessons
 *   file is present and current at that generation. Placing it while some of
 *   those files are still drifted/unreadable/deferred would make that assertion
 *   false — and, because the generation check trusts the marker over any content
 *   inference, would freeze those files as "current" forever. Withholding the
 *   marker for exactly those runs is what keeps the assertion honest.
 *
 *   `useTeam()` passes nothing here: a fresh apply has no drift to defer, so the
 *   marker is placed exactly as before.
...
export function scaffoldTeamLessons(destDir, scaffoldDir = TEAM_LESSONS_SCAFFOLD_DIR, opts = {}) {
  const { dryRun = false, skipRelPaths } = opts;
  // Normalized to a Set of forward-slash paths ONCE, here: `walk()` builds its
  // relPath with path.join(), which is backslash-separated on Windows, so a
  // naive `skip.has(relPath)` would silently never match a nested path like
  // "scripts/template-version.mjs" on exactly the platform this feature was
  // reported from.
  const skip = new Set([...(skipRelPaths ?? [])].map((p) => p.split(path.sep).join('/')));
  const created = [];
  const preserved = [];
  ...
  const walk = (dir, relBase) => {
    ...
    for (const entry of entries) {
      ...
      // Withheld: neither created nor preserved, and absent from both arrays —
      // this call behaves as if the template never contained the file.
      if (skip.has(relPath.split(path.sep).join('/'))) continue;
...
export function useTeam(teamName, projectRoot = process.cwd(), opts = {}) {
  ...
  const lines = [
    ...
    `  Team lessons   : .claude/skills/${TEAM_LESSONS_SKILL_NAME}/ (${lessonsScaffold.created.length} file${lessonsScaffold.created.length !== 1 ? 's' : ''} created, ${lessonsScaffold.preserved.length} preserved — never overwritten by "use")`,
