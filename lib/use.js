/**
 * use.js — implements `ccpm use <team>`.
 *
 * All paths under the user's project are resolved from process.cwd() so that
 * running `ccpm use frontend` from any directory affects that directory's .claude/.
 *
 ...
 * that ccpm should not own. We create ./CLAUDE.md if it does not exist.
 *
 * settings.json target: <cwd>/.claude/settings.json (project-level settings).
 * ccpm only manages a single env key defined by AGENT_TEAMS_ENV. It JSON-merges
 ...
 *   1. The manifest (placedFiles array) tracks every file ccpm wrote, so on a
 ...
 *      incoming agent filename would overwrite a hand-written file that ccpm
 ...
 */
export function useTeam(teamName, projectRoot = process.cwd()) {
  // ── 0. Resolve the team ──────────────────────────────────────────────────
  ...
  // Build a Set of absolute paths that ccpm placed last time. Files in this
  ...
  // .claude/agents/ AND was NOT placed by ccpm last time (not in prevPlacedSet).
  ...
  if (collisions.length > 0) {
    ...
    return {
      success: false,
      message:
        `ccpm: refusing to overwrite hand-written agent(s): ${list}.\n` +
        `Rename or remove them, then retry.`,
    };
  }

  // ── 3. Remove previously ccpm-placed files ───────────────────────────────
  ...
  // OWNERSHIP RULE: ccpm only removes the key on switch if its OWN previous
  ...
  if (agentTeamsFlagSet) {
    lines.push(
      '',
      `  Agent teams   : ENABLED (${AGENT_TEAMS_ENV}=1 written to .claude/settings.json)`,
    );
  }
