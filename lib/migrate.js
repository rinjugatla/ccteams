 ...
 * scaffold-file check, a missing catalog-injection-hook check in settings.json,
 * and a ccteams-owned-file (agent/skill) upstream-diff check — see
 * teamLessonsTemplateStep, teamLessonsScaffoldStep, teamLessonsHookStep and
 * ownedFilesStep below), so that further steps can be added by appending to
 * MIGRATION_STEPS without touching migrate() itself. See the `id`/`title`/`run`
 * shape below — and MIGRATION_STEPS' own comment for why the ORDER of the first
 * two is load-bearing.
 *
