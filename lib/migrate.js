 ...
 * independent "migration step" objects rather than one big function (a
 * team-lessons template-generation check, an old-layout `SKILL.md` / missing
 * scaffold-file check, a missing-`applies_when` advisory scan over the user's
 * own `lessons/**`, a missing catalog-injection-hook check in settings.json,
 * and a ccteams-owned-file (agent/skill) upstream-diff check — see
 * teamLessonsTemplateStep, teamLessonsScaffoldStep, teamLessonsAppliesWhenStep,
 * teamLessonsHookStep and ownedFilesStep below), so that further steps can be
 * added by appending to MIGRATION_STEPS without touching migrate() itself. See
 * the `id`/`title`/`run` shape below — and MIGRATION_STEPS' own comment for why
 * the ORDER of the first two is load-bearing.
 *
 * Interactivity: ownedFilesStep and teamLessonsTemplateStep are the two steps
 * that may prompt (via
...
import { hashFileSync, hashFileNormalizedSync } from './hash.js';
...
// reasoning as the import in use.js). parseFrontmatter is reused rather than
// reimplemented so teamLessonsAppliesWhenStep reads `applies_when` with the
// exact same rules gen-lessons.mjs itself uses to build the index — see that
// step's own doc comment for why loadLessons (also exported from the same
// file) is deliberately NOT used here instead.
import { CATALOG_START, CATALOG_END, parseFrontmatter } from '../scaffold/team-lessons/scripts/gen-lessons.mjs';
 ...
 * force, promptFn, teamLessonsTemplate }. `yes`/`force` mirror migrate()'s own
 * opts (see its doc comment); `promptFn` is `(question: string) =>
 * Promise<string>` or `null` when no step may prompt this run (dry-run,
 * `--yes`, non-TTY, or a test that injected `prompt: null`) — read by
 * ownedFilesStep and teamLessonsTemplateStep. `teamLessonsTemplate` is the
 * always-resolved `{ scaffoldDir, ledger, packageVersion }` triple
 * teamLessonsTemplateStep works from (defaults filled in by migrate()).
 * `withheldTeamLessonsFiles` is a `Set<string>` of skill-relative,
 * forward-slash paths teamLessonsScaffoldStep must not place — written by
 * teamLessonsTemplateStep, read by teamLessonsScaffoldStep, always present (see
 * migrate()'s ctx construction and THE MARKER INVARIANT below).
 *
 ...
 * something" — changing that is out of scope here. teamLessonsHookStep and
 * teamLessonsAppliesWhenStep are the clearest examples: both are pure
 * detect-and-report steps whose `added`/`updated` are always empty.
 */

/**
 * Copy `src` over `dest`, wrapping any fs failure (e.g. EACCES on a read-only
 * .claude/) in a short, path-specific Error instead of letting the raw error
 * and its stack reach the user — migrate()'s own catch turns that into
 * `{ success:false, message }` exactly like any other step failure.
 *
 * Shared by ownedFilesStep and teamLessonsTemplateStep so both report an
 * unwritable destination with the identical sentence.
 *
 * @param {string} src absolute path to the package's copy
 * @param {string} dest absolute path in the project
 * @param {string} disp display path used in the error message
 */
function writeOwnedFile(src, dest, disp) {
  try {
...
/**
 * The one ccteams-owned team-lessons file that is pure ccteams bookkeeping
 * rather than tooling the user might legitimately have adjusted — see the
 * per-file branch in teamLessonsTemplateStep for why it gets its own rule.
 *
 * THE MARKER INVARIANT (the rule every write of this file must respect):
 *   A generation marker on disk asserts that EVERY ccteams-owned team-lessons
 *   file is present and current at that generation. It may therefore never be
 *   written — neither advanced by teamLessonsTemplateStep nor placed by
 *   teamLessonsScaffoldStep — while any owned file is still drifted, unreadable,
 *   or deferred by the user.
 *
 * Violating it is not a cosmetic reporting bug: the generation check trusts the
 * marker over any content inference (see classifyTemplateGeneration), so a
 * marker written next to a stale file makes every later run return 'current'
 * and the stale file can never be updated again — not even with
 * `--yes --force`. That was a real, reproduced regression, which is why the
 * invariant is stated here and enforced in TWO places (this step's own marker
 * branch, and the withheld-paths set handed to the scaffold step).
 */
const TEAM_LESSONS_MARKER_FILE = 'scripts/template-version.mjs';

/**
 * One line naming EXACTLY what evidence identified this install as outdated —
 * and nothing more. Each branch may only state what its own detection
 * mechanism actually established (the same rule buildOwnedFilesNotices and
 * classifySkillLayout follow elsewhere in this file):
 *   - a marker was read           → both generation numbers are known facts
 *   - the ledger matched          → the generation is known, but only by hash
 *   - the signature fired         → only "older than generation N" is known
 *   - nothing could be verified   → say exactly that, and claim nothing else
 *
 * This is also what makes the three-tier detection observable to a user (and
 * therefore testable): without it, all four outdated-* values took a single
 * indistinguishable code path.
 *
 * @param {{ generation: string, markerVersion: number|null, packageVersion: number,
 *   ledgerVersion: number|null }} facts
 * @returns {string}
 */
function describeGenerationEvidence({ generation, markerVersion, packageVersion, ledgerVersion }) {
  if (generation === 'outdated-marker') {
    return `  This project's team-lessons template is generation ${markerVersion}; ccteams ships generation ${packageVersion}.`;
  }
  if (generation === 'outdated-ledger') {
    return (
      `  This project's team-lessons template has no generation marker; its file contents identify it` +
      ` as generation ${ledgerVersion}. ccteams ships generation ${packageVersion}.`
    );
  }
  if (generation === 'outdated-signature') {
    return (
      `  This project's team-lessons template has no generation marker, and its scripts/gen-lessons.mjs` +
      ` never mentions "${OUTDATED_SIGNATURE}", so it predates generation ${OUTDATED_SIGNATURE_INTRODUCED_IN}.` +
      ` ccteams ships generation ${packageVersion}.`
    );
  }
  // 'outdated-unknown': outdated by construction (every install that ships the
  // marker HAS the marker), but nothing about the content was verified — so the
  // sentence must not imply any generation number for it.
  return (
    `  This project's team-lessons template has no generation marker, so it predates ccteams'` +
    ` generation tracking (ccteams ships generation ${packageVersion}). Its contents match no version` +
    ' ccteams shipped, so nothing further about it could be verified.'
  );
}

/**
 * Build teamLessonsTemplateStep's notices (one rendered line per element, per
 * the notices contract above).
 *
 * As everywhere else in this file, each heading names ONLY what its own branch
 * established. In particular the drift block must not say "you edited these
 * files": all that branch verified is that the content matches no version
 * ccteams ever shipped — which a formatter run produces just as readily as a
 * hand edit (Issue #60 measured prettier changing 245 of gen-lessons.mjs's 321
 * lines). That ambiguity is precisely why the block spells out the
 * formatter case and the `--force` escape hatch.
 *
 * @param {{ generation: string, markerVersion: number|null, packageVersion: number,
 *   ledgerVersion: number|null, unreadableProjectDisp: string[],
 *   unreadablePackageDisp: string[], driftedDisp: string[],
 *   markerWithheld: boolean }} findings
 * @returns {string[]}
 */
function buildTeamLessonsTemplateNotices({
  generation,
  markerVersion,
  packageVersion,
  ledgerVersion,
  unreadableProjectDisp,
  unreadablePackageDisp,
  driftedDisp,
  markerWithheld,
  heldBackDisp = [],
}) {
  ...
  if (generation === 'newer') {
    notices.push(
      `! This project's team-lessons template is generation ${markerVersion}, which is NEWER than the`,
      `  generation ${packageVersion} template the installed ccteams ships — nothing was changed.`,
    );
    // Emitted ONLY when files were genuinely held back, and naming them: on a
    // project that already has every owned file, withholding changed nothing,
    // and claiming otherwise would describe an action that never happened.
    if (heldBackDisp.length > 0) {
      notices.push(
        `  ccteams also held back ${heldBackDisp.length} file(s) it would normally add, rather than`,
        '  splicing an older generation into a newer install:',
        ...heldBackDisp.map((p) => `    ${p}`),
      );
    }
    notices.push(
      '  This happens after downgrading ccteams. Upgrade it again',
      '  ("npm install -g https://github.com/rinjugatla/ccteams.git"), then re-run "ccteams migrate".',
    );
    return notices;
  }

  const hasFinding =
    unreadableProjectDisp.length > 0 || unreadablePackageDisp.length > 0 || driftedDisp.length > 0;
  if (!hasFinding) return notices;

  // Stated once, up front, so every block below is read against the evidence
  // that actually produced it.
  notices.push(
    '! Some ccteams-owned team-lessons files were left as they are.',
    describeGenerationEvidence({ generation, markerVersion, packageVersion, ledgerVersion }),
  );

  if (unreadablePackageDisp.length > 0) {
    notices.push(
      `  ${unreadablePackageDisp.length} file(s) could not be read from CCTEAMS' OWN copy of the template,`,
      '  so there was nothing to compare your project against:',
      ...unreadablePackageDisp.map((p) => `    ${p}`),
      "  This is a problem with the ccteams installation, not with your project — no file of yours",
      '  is at fault. Reinstall ccteams ("npm install -g https://github.com/rinjugatla/ccteams.git"),',
      '  then re-run "ccteams migrate".',
    );
  }

  if (unreadableProjectDisp.length > 0) {
    notices.push(
      `  ${unreadableProjectDisp.length} file(s) in this project could not be read (permission error or`,
      '  similar) — left untouched:',
      ...unreadableProjectDisp.map((p) => `    ${p}`),
  ...
  if (driftedDisp.length > 0) {
    notices.push(
      `  ${driftedDisp.length} file(s) no longer match any version ccteams shipped — left unchanged:`,
      ...driftedDisp.map((p) => `    ${p}`),
      '  If the only difference is formatting (e.g. this project runs prettier over ".claude/**"),',
      '  "ccteams migrate --yes --force" updates them safely — re-run your formatter afterwards to',
      "  restore the project's formatting.",
      '  Otherwise, run "ccteams migrate" in an interactive terminal to decide file by file.',
    );
  }

  if (markerWithheld) {
    // Says WHY nothing looks like it changed, and — more importantly — promises
    // the finding survives, which is the whole point of withholding the marker.
    notices.push(
      `  ccteams deliberately did not write ${TEAM_LESSONS_MARKER_FILE}: recording this generation`,
      '  while the files above are unresolved would make every later run report "up to date" and',
      '  freeze them for good. It will be recorded once they are resolved; until then this note',
...
/**
 * Migration step: refresh the ccteams-OWNED files inside the otherwise
 * user-owned team-lessons skill — `AUTHORING.md` and `scripts/**` (see
 * TEAM_LESSONS_OWNED_FILES in lib/team-lessons-template.js). `SKILL.md` and
 * `lessons/**` are the user's and are never read, written, or reported by this
 * step.
 *
 * WHY THIS STEP EXISTS (Issue #60). ownedFilesStep's scope is `placedFiles`,
 * and team-lessons is structurally absent from every team's `placedFiles` (see
 * that step's SCOPE note), so before this step nothing in
 * `.claude/skills/team-lessons/` could EVER be updated — a project that
 * installed the skill once was frozen on that generation forever, `--force`
 * included.
 *
 * WHY IT MUST RUN FIRST (see MIGRATION_STEPS). `scripts/template-version.mjs`
 * is a NEW file, so teamLessonsScaffoldStep — whose contract is "place every
 * template file the destination lacks" — would place the CURRENT marker into a
 * project whose other files are ancient. The generation check would then read
 * that freshly-written marker, conclude 'current', and leave the stale files
 * alone forever. Hence the required order: check the generation, update, and
 * only THEN let the scaffold step add what is missing.
 *
 * Files this step never adds: a ccteams-owned file that is ABSENT from the
 * project is deliberately skipped here and left to teamLessonsScaffoldStep,
 * which already adds missing template files and reports them under `added`.
 * Adding them here too would double-report the same file in one run.
 *
 * Overwrite policy per file, once the install is known to be outdated:
 *   - content already equal to the package's         → nothing at all
 *   - the marker file, when it parsed                → refreshed silently: a
 *     parsable marker is ccteams' own bookkeeping, never user prose
 *   - content equal to SOME generation ccteams shipped → refreshed silently:
 *     the copy is verifiably unedited, so nothing of the user's is at risk
 *   - anything else                                   → SKIPPED and reported.
 *     Overwriting needs `--yes --force`, or an interactive 'y'/'a' — the same
 *     bar ownedFilesStep sets for its user-modified files.
 */
export const teamLessonsTemplateStep = {
  id: 'team-lessons-template',
  // Must differ from teamLessonsScaffoldStep's 'team-lessons skill': the two
  // steps both report on the same directory, and an identical title would
  // print the same heading twice in one report.
  title: 'team-lessons template files',
  async run(ctx) {
    const destDir = path.join(ctx.dotClaudeDir, 'skills', TEAM_LESSONS_SKILL_NAME);
    const empty = { id: this.id, title: this.title, added: [], updated: [], kept: [], notices: [] };

    // Nothing to reconcile when the skill is not installed — and when a plain
    // FILE squats on the destination, teamLessonsScaffoldStep owns the error
    // message (it is the step that would fail to write). Duplicating or
    // pre-empting either case here would change which step reports it.
    if (!fs.existsSync(destDir) || !fs.statSync(destDir).isDirectory()) return empty;

    const { scaffoldDir, ledger, packageVersion } = ctx.teamLessonsTemplate;
    const absIn = (base, rel) => path.join(base, ...rel.split('/'));
    // Report rows and notices use the SKILL-relative path (`scripts/x.mjs`) —
    // the `team-lessons template files` heading right above them supplies the
    // context, and that is the convention teamLessonsScaffoldStep already uses.
    // The interactive PROMPT gets the PROJECT-relative path instead: it is
    // printed standalone on stdin with no heading, in the same session where
    // ownedFilesStep asks about `.claude/agents/builder.md`, so a bare
    // `scripts/gen-lessons.mjs` would leave the user guessing which file is
    // even under discussion.
    const displayDestDir = path.relative(ctx.projectRoot, destDir).split(path.sep).join('/');
    const promptPathFor = (rel) => `${displayDestDir}/${rel}`;

    const markerVersion = readTemplateVersion(absIn(destDir, TEAM_LESSONS_MARKER_FILE));
    const diskHashes = new Map(
      TEAM_LESSONS_LEDGER_FILES.map((rel) => [rel, hashFileNormalizedSync(absIn(destDir, rel))]),
    );
    let genLessonsText = null;
    try {
      genLessonsText = fs.readFileSync(absIn(destDir, 'scripts/gen-lessons.mjs'), 'utf8');
    } catch {
      genLessonsText = null; // absent/unreadable — establishes nothing either way
    }

    // Kept in a local (rather than inlined into the call) because the notices
    // report the generation the ledger identified, not just the fact that it
    // matched — see describeGenerationEvidence.
    const ledgerVersion = findLedgerVersion(diskHashes, ledger);
    const generation = classifyTemplateGeneration({
      markerVersion,
      packageVersion,
      ledgerVersion,
      signatureIsOld: signatureIsOutdated(genLessonsText),
    });

    if (generation === 'current') return empty;
    if (generation === 'newer') {
      // Withhold EVERY ccteams-owned file from teamLessonsScaffoldStep. Its
      // contract is "place every template file the destination lacks", and this
      // package's template is OLDER than what the project has — back-filling a
      // missing owned file from it would splice an older generation into a newer
      // install, which is precisely the state 'newer' exists to prevent.
      // SKILL.md and lessons/.gitkeep are deliberately NOT withheld: they are
      // user-owned, carry no generation, and placing them is still correct.
      for (const rel of TEAM_LESSONS_OWNED_FILES) ctx.withheldTeamLessonsFiles.add(rel);
      // Only the files the project actually LACKS were held back in any
      // meaningful sense — withholding a path that is already on disk changes
      // nothing. The notice names exactly those, and says nothing at all when
      // the set is empty, so it never claims an action that did not happen.
      const heldBackDisp = TEAM_LESSONS_OWNED_FILES.filter(
        (rel) => !fs.existsSync(absIn(destDir, rel)),
      );
      return {
        ...empty,
        notices: buildTeamLessonsTemplateNotices({
          generation,
          markerVersion,
          packageVersion,
          ledgerVersion,
          ...
          unreadablePackageDisp: [],
          driftedDisp: [],
          markerWithheld: false,
          heldBackDisp,
        }),
      };
    }

    // Every remaining generation is an 'outdated-*' one — see
    // classifyTemplateGeneration()'s docstring for why even 'outdated-unknown'
    // is outdated by construction.
    const updated = [];
    const unreadableProjectDisp = [];
    const unreadablePackageDisp = [];
    const driftedDisp = [];
    let markerWithheld = false;
    // Interactive "answer for the rest of the run" state, mirroring
    // ownedFilesStep's own loop.
    let overwriteAllRemaining = false;
    let skipAllRemaining = false;

    // THE MARKER INVARIANT, half one (see TEAM_LESSONS_MARKER_FILE's comment):
    // true only while every owned file seen so far ended up current.
    const everythingResolvedSoFar = () =>
      driftedDisp.length === 0 &&
      unreadableProjectDisp.length === 0 &&
      unreadablePackageDisp.length === 0;

    for (const rel of TEAM_LESSONS_OWNED_FILES) {
      const src = absIn(scaffoldDir, rel);
      // Defensive: a package that does not ship this file has nothing to
      // update from (and nothing to compare against).
      if (!fs.existsSync(src)) continue;

      const dest = absIn(destDir, rel);
      if (!fs.existsSync(dest)) continue; // teamLessonsScaffoldStep's job — see above

      // NO DOUBLE-REPORTING. This file exists AND this run is an outdated-* one,
      // so it is entirely this step's business: refreshes become `~ (updated)`
      // rows here, and skipped/unreadable ones are named in this step's notices.
      // Left un-withheld, teamLessonsScaffoldStep would ALSO list it as
      // `= (kept as-is)` — so one report would say a file was updated and, four
      // lines later, that it was kept, with no way for the user to tell which.
      // A file that is already current is reported by neither, matching
      // ownedFilesStep's own "unchanged files are never reported" rule.
      //
      // Only EXISTING files are withheld: an ABSENT owned file must still be
      // placed and announced as `+ (added)` by the scaffold step (that is how a
      // brand-new scripts/template-version.mjs legitimately appears), and the
      // 'current' generation never reaches this loop at all, so its familiar
      // `= AUTHORING.md (kept as-is)` output is unchanged.
      ctx.withheldTeamLessonsFiles.add(rel);

      const uHash = hashFileNormalizedSync(src);
      const pHash = hashFileNormalizedSync(dest);
      // Split deliberately, never collapsed into one "unreadable" bucket: these
      // are failures of DIFFERENT files owned by DIFFERENT parties, and telling
      // a user to check permissions on their own file when it is ccteams' own
      // installation that is broken sends them to fix the wrong thing. (Same
      // distinction classifyOwnedFile()'s doc comment insists on for its own
      // `unreadable` state.)
      if (uHash === null) {
        unreadablePackageDisp.push(rel);
        continue;
      }
      if (pHash === null) {
        unreadableProjectDisp.push(rel);
        continue;
      }
      if (pHash === uHash) continue; // already current, whatever the marker said

      if (rel === TEAM_LESSONS_MARKER_FILE) {
        // THE MARKER INVARIANT, half one. The marker sorts LAST in
        // TEAM_LESSONS_OWNED_FILES (asserted in the test suite, precisely so
        // this stays true rather than holding by alphabetical accident), so by
        // the time the loop reaches it every other owned file's outcome is
        // already decided and this single check is sufficient.
        //
        // Advancing the marker here while another file is drifted/unreadable
        // would be the SAME freeze bug the withheld-paths set prevents for a
        // missing marker — just reached from a project that already had one.
        if (!everythingResolvedSoFar()) {
          markerWithheld = true;
          continue;
        }
      }
      ...
      const safeToRefresh =
        rel === TEAM_LESSONS_MARKER_FILE
          ? // A parsable marker is ccteams' own bookkeeping, never user prose,
            // so refreshing it needs no confirmation. An UNPARSABLE one
            // (markerVersion === null) is not something ccteams can vouch for —
            // it falls through to the drift path like any other file.
            markerVersion !== null
          : isKnownShippedHash(rel, pHash, ledger);

      if (safeToRefresh) {
        if (!ctx.dryRun) writeOwnedFile(src, dest, rel);
        updated.push(rel);
        continue;
      }

      // The file is outdated, but its content matches no version ccteams
      // shipped — a real edit or a reformat, and this step cannot tell them
      // apart. Default to keeping it.
      let overwrite = false;
      if (ctx.yes && ctx.force) {
        ...
        overwrite = overwriteAllRemaining
          ? true
          : await askAboutFile(ctx, 'template-outdated', promptPathFor(rel), dest, src, {
              onAll: () => {
      ...
      if (overwrite) {
        ...
        driftedDisp.push(rel);
      }
    }

    // THE MARKER INVARIANT, half two (see TEAM_LESSONS_MARKER_FILE's comment).
    // Half one above stops this step from ADVANCING an existing marker; this
    // stops teamLessonsScaffoldStep from PLACING a missing one. Both are needed:
    // a pre-marker project hits only the second, a marker-bearing project only
    // the first.
    //
    // Computed from the same classification and the same per-file outcomes in
    // BOTH modes — nothing here reads ctx.dryRun — so `migrate --dry-run` and a
    // real run always withhold exactly the same paths, and therefore always
    // report the same `added` set. That agreement is a hard requirement of this
    // module (see scaffoldTeamLessons's dry-run contract), not an accident.
    //
    // Consequence, intended: a user who deliberately keeps their own version of
    // a template file is re-notified on EVERY run, because the marker is never
    // recorded while that file is outstanding. This mirrors ownedFilesStep,
    // which likewise re-reports a user-modified file every run rather than
    // remembering a "don't ask again" answer — the alternative is silently
    // dropping a finding the user never resolved.
    if (!everythingResolvedSoFar()) {
      ctx.withheldTeamLessonsFiles.add(TEAM_LESSONS_MARKER_FILE);
      markerWithheld = true;
    }
...
export const teamLessonsScaffoldStep = {
  ...
  run(ctx) {
    ...
    let result;
    try {
      result = scaffoldTeamLessons(destDir, undefined, {
        dryRun: ctx.dryRun,
        // THE MARKER INVARIANT (see TEAM_LESSONS_MARKER_FILE in this file):
        // teamLessonsTemplateStep runs FIRST and fills this Set with the paths
        // this step must NOT place — the generation marker while another owned
        // file is still unresolved, or every owned file when the project's
        // template is NEWER than this package's. A withheld path is not
        // reported as added OR kept, so this step never claims to have handled
        // a file it deliberately left alone.
        skipRelPaths: ctx.withheldTeamLessonsFiles,
      });
    } catch (err) {
...
/**
 * Build teamLessonsAppliesWhenStep's notices (one rendered line per element,
 * per the notices contract above).
 *
 * Two independent findings, each its own `!` block, because they name
 * different, mutually exclusive facts and need different fixes:
 *   - missingDisp: the file's frontmatter parsed fine, but `applies_when` is
 *     absent, non-string (the array form `applies_when: [a, b]`), or blank —
 *     gen-lessons.mjs's own `loadLessons` falls back to the legacy
 *     symptom-only index heading for these rather than failing the build
 *     (see its doc comment), so this is advisory, not broken.
 *   - unreadableDisp: the file itself, or its frontmatter block, could not be
 *     read/parsed at all. This is a DIFFERENT, more urgent fact —
 *     gen-lessons.mjs's `loadLessons` throws outright on a lesson with no
 *     frontmatter block, failing the whole index build — and this branch
 *     never established anything about `applies_when` specifically, so it
 *     must not be folded into missingDisp (the file header's "a heading may
 *     only claim what its own branch established" rule, applied here too).
 *
 * @param {{ missingDisp: string[], unreadableDisp: string[], displayDestDir: string }} findings
 * @returns {string[]}
 */
function buildTeamLessonsAppliesWhenNotices({ missingDisp, unreadableDisp, displayDestDir }) {
  const notices = [];
  const generatorCommand = `node ${displayDestDir}/scripts/gen-lessons.mjs`;

  if (missingDisp.length > 0) {
    notices.push(
      `! ${missingDisp.length} lesson(s) have no "applies_when" in their frontmatter — the index falls`,
      '  back to the older, symptom-only heading for them instead of the "when to read this" one:',
      ...missingDisp.map((p) => `    ${p}`),
      `  Ask Claude Code to read ${displayDestDir}/AUTHORING.md and fill in`,
      '  "applies_when: <when to read this>" for each, following the frontmatter schema there',
      '  (you may also write it by hand). Then regenerate the index:',
      `    ${generatorCommand}`,
      '  (or your project\'s own alias, e.g. "pnpm run gen:lessons")',
    );
  }
  ...
  if (unreadableDisp.length > 0) {
    notices.push(
      `! ${unreadableDisp.length} lesson(s) could not be checked for "applies_when":`,
      ...unreadableDisp.map((p) => `    ${p}`),
      '  Each could not be read, or has no frontmatter block (a leading `---`-delimited block) —',
      '  "gen-lessons.mjs" fails outright on a lesson like that, so this must be fixed by hand',
      `  before the index can be regenerated. See ${displayDestDir}/AUTHORING.md for the`,
      '  frontmatter schema.',
    );
  }
...
/**
 * Migration step: scan `lessons/**` (entirely user-owned — never read or
 * written by any OTHER step; see teamLessonsScaffoldStep's own doc comment)
 * for lesson files missing a usable `applies_when`, and REPORT, never write,
 * what needs attention. Purely advisory — see this step's own `added`/
 * `updated` contract note below.
 *
 * Placed after teamLessonsScaffoldStep in MIGRATION_STEPS (see that array's
 * own comment for why the position matters): scaffolding only ever places
 * `lessons/.gitkeep`, never a `.md` file, so this step's findings are
 * unaffected by running before or after it — but running after keeps every
 * step that inspects `.claude/skills/team-lessons/` looking at that
 * directory in its FINAL state for this run, matching the convention
 * teamLessonsHookStep's own DESIGN-D note establishes for the sibling hook
 * step below.
 *
 * Detection reaches `applies_when` only; `id`/`symptom`/`summary` are
 * deliberately NOT validated here, unlike gen-lessons.mjs's own
 * `loadLessons`, which throws on a missing/empty one of those. Reusing
 * `loadLessons` would let a single malformed lesson (e.g. an empty
 * `symptom`) abort `migrate()` entirely (issue #61's completion condition
 * that migrate() must not die on one bad lesson) — this step instead goes
 * straight to `parseFrontmatter` (also exported from gen-lessons.mjs), which
 * throws in exactly ONE narrower case: the frontmatter BLOCK itself cannot be
 * parsed (no leading `---`-delimited region). It never throws on a missing or
 * invalid `id`/`symptom`/`summary` — those are `loadLessons`'s own checks,
 * layered on top of `parseFrontmatter`'s return value, not something
 * `parseFrontmatter` itself enforces. So a lesson with a bad `id` but a
 * parsable frontmatter block parses here without incident (see the
 * malformed-lesson test), and the one case `parseFrontmatter` DOES throw on
 * is caught immediately below and routed into the unreadable group, never
 * left to propagate.
 */
export const teamLessonsAppliesWhenStep = {
  id: 'team-lessons-applies-when',
  // Distinct from teamLessonsScaffoldStep's 'team-lessons skill' and
  // teamLessonsTemplateStep's 'team-lessons template files': all three report
  // on the same directory, and a repeated title would print a duplicate
  // heading in one report.
  title: 'team-lessons lessons',
  run(ctx) {
    const skillDir = path.join(ctx.dotClaudeDir, 'skills', TEAM_LESSONS_SKILL_NAME);
    const lessonsDir = path.join(skillDir, 'lessons');
    const displayDestDir = path.relative(ctx.projectRoot, skillDir).split(path.sep).join('/');
    const displayPath = (abs) => path.relative(ctx.projectRoot, abs).split(path.sep).join('/');
    // added/updated are always empty: this step only ever READS lessons/**
    // and never writes to it (see the notices contract's "advisory, never
    // work ccteams performed or will perform" rule above). `updated` is set
    // explicitly here — rather than relying on formatMigrateReport's `?? []`
    // fallback — so a caller inspecting the step result directly still sees
    // the same guarantee.
    const empty = { id: this.id, title: this.title, added: [], updated: [], kept: [], notices: [] };

    // A brand-new install (or one where the user has not written a lesson
    // yet) has no lessons/ directory at all, or one holding only the
    // scaffolded `.gitkeep` — nothing to check either way. Returning the
    // identical empty result regardless of dryRun keeps this step in
    // agreement with every other step's dry-run/real-run contract (neither
    // mode writes anything here to begin with).
    if (!fs.existsSync(lessonsDir)) return empty;

    let entries;
    try {
      entries = fs.readdirSync(lessonsDir);
    } catch (err) {
      // EACCES, or ENOTDIR when something other than a directory squats on
      // `lessons/` — never thrown onward (the same "a step must detect its
      // own failure conditions" convention teamLessonsScaffoldStep's own
      // non-directory guard already follows). Collapsed into one notice,
      // mirroring readSettingsForHookDetection's read+parse collapse above:
      // the caller's advice ("fix it, then re-run") is identical whichever
      // fs error actually fired.
      return {
        ...empty,
        notices: [
          `! ${displayDestDir}/lessons/ exists but could not be listed (${err.code ?? err.message}) —`,
          '  could not check whether "applies_when" is set for its lessons.',
          // Cause-neutral: the two causes this branch actually collapses (see
          // the comment above) are not both permissions problems — ENOTDIR
          // means something other than a directory is sitting at that path,
          // which "fix the permissions" would not address. err.code is
          // already in the heading line above, so naming the specific cause
          // here would only repeat it, not add information.
          '  Make sure it is a readable directory, then re-run "ccteams migrate".',
        ],
      };
    }
    ...
    for (const name of entries.filter((n) => n.endsWith('.md')).sort()) {
      const abs = path.join(lessonsDir, name);
      let frontmatter;
      try {
        frontmatter = parseFrontmatter(fs.readFileSync(abs, 'utf8'));
      } catch {
        // Unreadable file, or no frontmatter block at all — this branch never
        // established anything about `applies_when` specifically, so it must
        // NOT be folded into missingDisp (see
        // buildTeamLessonsAppliesWhenNotices's doc comment on why the two
        // findings are kept separate).
        unreadableDisp.push(displayPath(abs));
        continue;
      }

      // Non-string covers both an absent key (`undefined`) and the array
      // form `applies_when: [a, b]` — gen-lessons.mjs's own `loadLessons`
      // only ever accepts a plain string
      // (`typeof frontmatter.applies_when === 'string'`), so anything else
      // is equally "not set" from the generated index's point of view.
      //
      // The explicit `.trim()` here is a defensive no-op against
      // parseFrontmatter's CURRENT behavior — verified directly: its value
      // extraction already does `line.slice(idx + 1).trim()`, so
      // `applies_when:    ` (whitespace, no content) already comes back as
      // `''` before this line ever runs, which is exactly why
      // gen-lessons.mjs's own untrimmed `if (!appliesWhen)` already treats it
      // as missing too. Trimming again here costs nothing and keeps this
      // check correct even if parseFrontmatter's own trimming were ever
      // loosened — it is not meant to catch a case gen-lessons.mjs misses.
      if (typeof frontmatter.applies_when !== 'string' || frontmatter.applies_when.trim() === '') {
        missingDisp.push(displayPath(abs));
      }
    }

    if (missingDisp.length === 0 && unreadableDisp.length === 0) return empty;

    return {
      ...empty,
      notices: buildTeamLessonsAppliesWhenNotices({ missingDisp, unreadableDisp, displayDestDir }),
    };
  },
};
 ...
 *
 * Each state gets DIFFERENT wording on purpose (see ownedFilesStep's own
 ...
 * 'template-outdated' (teamLessonsTemplateStep) has verified a third, narrower
 * thing: the install's team-lessons template is from an older GENERATION, and
 * this file's content matches no generation ccteams ever shipped — which a
 * formatter run explains as readily as a hand edit, so it must not claim
 * either.
 *
 * @param {'user-modified' | 'unknown-baseline' | 'template-outdated'} state
...
function buildOverwritePrompt(state, dispPath) {
  ...
  if (state === 'template-outdated') {
    return (
      `${dispPath} — from an older team-lessons template generation, and it differs from\n` +
      '  every version ccteams shipped, so ccteams cannot tell an edit of yours from a\n' +
      `  reformat (e.g. prettier).\n${options}`
    );
  }
  return (
...
/**
 * Ask the user what to do about one file needing a decision, looping on an
 * invalid answer or a diff request until a terminal answer (y/n/a/q) is given.
 *
...
export const ownedFilesStep = {
  ...
  async run(ctx) {
    ...
    for (const dest of sortedDests) {
      ...
      if (state === 'missing') {
        if (!dryRun) writeOwnedFile(src, dest, disp);
        added.push(disp);
 ...
 *   - teamLessonsTemplateStep MUST come first. `scripts/template-version.mjs`
 *     is a file older installs do not have, so teamLessonsScaffoldStep (which
 *     places every missing template file) would otherwise drop the CURRENT
 *     generation marker into a project whose other team-lessons files are
 *     ancient — after which the generation check reads its own freshly-written
 *     marker, reports 'current', and the stale files are never updated. Issue
 *     #60 states the required sequence as "check the generation → update →
 *     place missing files"; this array is where that sequence lives.
 *   - teamLessonsAppliesWhenStep must come after teamLessonsScaffoldStep.
 *     `lessons/**` is entirely user-owned detection territory — this step
 *     never places anything there (teamLessonsScaffoldStep only ever adds
 *     `lessons/.gitkeep`) — so it must run once scaffolding for this run has
 *     finished, the same "inspect the final state, not a half-updated one"
 *     reasoning teamLessonsHookStep's own DESIGN-D note gives for running
 *     after teamLessonsScaffoldStep too (see below).
 *   - teamLessonsHookStep must come after teamLessonsScaffoldStep (see the
 *     DESIGN-D note on lessonsIndexWillExist).
 */
export const MIGRATION_STEPS = [
  teamLessonsTemplateStep,
  teamLessonsScaffoldStep,
  teamLessonsAppliesWhenStep,
  teamLessonsHookStep,
  ownedFilesStep,
];
 ...
 *   teamLessonsTemplate ({ scaffoldDir?, ledger?, packageVersion? }) — a TEST
 *     SEAM for teamLessonsTemplateStep, in the same spirit as
 *     scaffoldTeamLessons()'s overridable `scaffoldDir` parameter. Historical
 *     template bytes cannot be reconstructed inside a test (they only exist in
 *     this repo's git history), so the 'outdated-ledger' path is unreachable
 *     without injecting a fake template directory plus a ledger describing it.
 *     Every field defaults to the real value, so an ordinary call — and every
 *     production code path — behaves exactly as if this option did not exist.
 *
 * Returns:
 * {
 *   success: true,
...
export async function migrate(projectRoot = process.cwd(), opts = {}) {
  const { dryRun = false, yes = false, force = false, prompt, teamLessonsTemplate } = opts;
  ...
  // Resolved once, here, so teamLessonsTemplateStep never has to know whether
  // a field was injected — it always reads three plain values off ctx.
  const templateOpts = {
    scaffoldDir: teamLessonsTemplate?.scaffoldDir ?? TEAM_LESSONS_SCAFFOLD_DIR,
    ledger: teamLessonsTemplate?.ledger ?? TEAM_LESSONS_HASH_LEDGER,
    packageVersion: teamLessonsTemplate?.packageVersion ?? packageTemplateVersion(),
  };
  ...
  const ctx = {
    ...
    force,
    promptFn,
    teamLessonsTemplate: templateOpts,
    // Cross-step channel, written by teamLessonsTemplateStep and read by
    // teamLessonsScaffoldStep (which runs immediately after it — see
    // MIGRATION_STEPS). Initialized here, never left undefined, so the scaffold
    // step needs no defensive check and a run where the template step
    // early-returns (no team-lessons directory: the fresh-install path, where
    // the marker SHOULD be placed) behaves exactly as it did before this
    // channel existed.
    withheldTeamLessonsFiles: new Set(),
  };
  let steps;
  try {
    steps = [];
  ...
  //
  // teamLessonsTemplateStep deliberately contributes NOTHING here: team-lessons
  // is never recorded in any team's placedFiles (see ownedFilesStep's SCOPE
  // note), and its generation is tracked by the on-disk marker file
  // (scripts/template-version.mjs), which IS the record. Adding a second,
  // manifest-side ledger for the same fact would just create two sources of
  // truth that can disagree after a manual edit of either.
