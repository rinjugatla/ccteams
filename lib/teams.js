/**
 * teams.js — discover and read team packages from the ccteams source repo.
 *
 * Teams live at <ccteams-repo-root>/teams/<team-name>/ and are resolved relative
 * to this file's location (import.meta.url), so the path works regardless of
 * where the user runs ccteams from (process.cwd is the user's project, not ours).
 */

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

// Resolve the ccteams source root from this file's location.
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const TEAMS_DIR = path.resolve(__dirname, '..', 'teams');

// Shared skills are shipped with ccteams at <repo-root>/shared/skills/<skillName>/.
export const SHARED_SKILLS_DIR = path.resolve(__dirname, '..', 'shared', 'skills');

// team-lessons is the USER-OWNED home for learning-loop entries (lessons the
// team accumulates in this project). The name is reserved: no team may ship a
// skill under it (see resolveTeamPlacement in placement.js, which filters it
// out of a team's requested skill names before any dest is computed). Defined
// here — rather than in use.js or placement.js, both of which need it — so
// there is a single source of truth both modules import from without creating
// a cycle (placement.js is imported BY use.js, so use.js cannot be the source).
export const TEAM_LESSONS_SKILL_NAME = 'team-lessons';

/**
 * Resolve the source directory for a skill by name.
 * Lookup order:
 *   1. <team.teamDir>/skills/<skillName>  — team-local override
 *   2. <repo-root>/shared/skills/<skillName> — package-level shared skill
 * Returns the first existing directory path, or null if neither exists.
 */
export function resolveSkillDir(team, skillName) {
  const teamLocal = path.join(team.teamDir, 'skills', skillName);
  if (fs.existsSync(teamLocal)) return teamLocal;
  const shared = path.join(SHARED_SKILLS_DIR, skillName);
  if (fs.existsSync(shared)) return shared;
  return null;
}

/**
 * Return an array of all available team descriptors.
 * Each element: { name, description, tags, teamDir }
 */
export function listTeams() {
  if (!fs.existsSync(TEAMS_DIR)) {
    return [];
  }

  const entries = fs.readdirSync(TEAMS_DIR, { withFileTypes: true });
  const teams = [];

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const teamDir = path.join(TEAMS_DIR, entry.name);
    const jsonPath = path.join(teamDir, 'team.json');
    if (!fs.existsSync(jsonPath)) continue;

    let meta;
    try {
      meta = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
    } catch {
      // Skip malformed team packages silently — user-visible error would be noise.
      continue;
    }

    teams.push({
      name: meta.name ?? entry.name,
      // Short one-line label for `list`; falls back to the description.
      summary: meta.summary ?? meta.description ?? '',
      description: meta.description ?? '',
      tags: meta.tags ?? [],
      // Optional: signals that CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS is required.
      requiresAgentTeams: meta.requiresAgentTeams === true,
      // Optional: extra skill names to install alongside the always-included working-method.
      skills: Array.isArray(meta.skills) ? meta.skills : [],
      teamDir,
    });
  }

  return teams;
}

/**
 * Return a single team descriptor by name, or null if not found.
 */
export function findTeam(name) {
  return listTeams().find((t) => t.name === name) ?? null;
}
