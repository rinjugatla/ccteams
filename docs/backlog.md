...
- Today ccpm applies ONE team per project (`ccpm use` is an exclusive switch, like nvm).
...
- So "frontend team in apps/web, backend team in apps/api, simultaneously and isolated" is
  NOT possible in Claude Code regardless of what ccpm does.
- Possible future approaches if demand appears (A and B converge to the same end state — all
  ...
  - B. Multi-team compose: `ccpm use frontend backend` merges both teams' agents into
