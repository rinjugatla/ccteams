...
3. The `.claude/skills/team-lessons/` skill is scaffolded (`SKILL.md`, `AUTHORING.md`, `scripts/gen-lessons.mjs`, `scripts/lessons-index.mjs`, `scripts/template-version.mjs`, `lessons/.gitkeep`). Each file is written **only if absent**: everything already there is left exactly as it is, nothing is ever tracked in the manifest or deleted, and the skill survives team applies, removals, re-applies, and package updates. Your lessons (`SKILL.md`, `lessons/**`) are never rewritten by anything, ever; the tooling ccteams ships alongside them (`AUTHORING.md`, `scripts/**`) can be refreshed later by `ccteams migrate` — see [Keeping a project up to date](#keeping-a-project-up-to-date-ccteams-migrate). (The name `team-lessons` is reserved — teams cannot ship a skill under it.) See [The team-lessons skill](#the-team-lessons-skill).
...
- **Project-specific lessons** go into the `.claude/skills/team-lessons/` skill — your `SKILL.md` and `lessons/**` there are yours alone and are never rewritten. The skill survives applying, removing, and re-applying teams, and package updates, and the orchestrator injects its entries into delegations alongside playbook rules. (Never put lessons in the playbook copies themselves — those are replaced on every `ccteams use`.)
...
`.claude/skills/team-lessons/` is where the learning loop's accepted entries land. It is scaffolded as six files:

```
.claude/skills/team-lessons/
├── SKILL.md                     # YOURS — generated index, one short entry per lesson
├── AUTHORING.md                 # ccteams' — frontmatter schema + how to add an entry
├── lessons/                     # YOURS — one lesson per file: NN-slug.md
└── scripts/                     # ccteams'
    ...
    └── template-version.mjs     # which generation of this tooling you have (see migrate)
```

**Who owns what.** `SKILL.md` and `lessons/**` are yours: they hold the knowledge this project accumulated, and ccteams never rewrites, overwrites, or deletes them under any command or flag. `AUTHORING.md` and `scripts/**` are ccteams' own authoring rules and tool implementation — not content you write — so `ccteams migrate` can bring them up to date when a newer ccteams fixes or extends them. See [Keeping a project up to date](#keeping-a-project-up-to-date-ccteams-migrate) for exactly how, including how it protects a copy you *did* edit.
...
- **What it does today:** six things, layered so nothing that could lose your work happens without your say-so.
  1. It **updates the ccteams-owned files inside `.claude/skills/team-lessons/`** — `AUTHORING.md` and `scripts/**` (`gen-lessons.mjs`, `lessons-index.mjs`, `template-version.mjs`). Your `SKILL.md` and `lessons/**` are **never** read, written, or reported by *this step* — see step 4 below for the one place `lessons/**` frontmatter is read (never written): an older ccteams stored every lesson in `SKILL.md` itself, so overwriting it would destroy exactly the knowledge the skill exists to keep. How the decision is made:
     - `scripts/template-version.mjs` holds a **generation number** — a plain integer, bumped only when a ccteams-owned team-lessons file actually changes, deliberately not tied to the package version (otherwise every release would rewrite these files for nothing). ccteams compares the number in your project against the one it ships: equal → nothing happens at all; lower → the files are brought up to date; higher (you downgraded ccteams) → nothing is changed and a note tells you to upgrade again.
     - A project installed **before** that marker existed has no number to read, so ccteams falls back to identifying the generation from the files' content — first against a ledger of every version it has shipped, then, for a copy a formatter has reflowed, by looking for a feature marker in the generator that no formatter and no user would remove.
     - A file it cannot verify as an untouched shipped version (a real edit — or a reformat, which looks the same from the outside) is **left alone and reported**, never overwritten silently. The note spells out the formatter case: if the only difference is formatting, `ccteams migrate --yes --force` updates the files safely — re-run your formatter afterwards to restore the project's style. Or run `ccteams migrate` in a terminal and decide file by file.
  2. It **adds** any file missing from the scaffolded `.claude/skills/team-lessons/` skill (see above) — the same never-overwrite scaffold `ccteams use` runs, so a project that only ran `ccteams use` a while ago can pick up files a newer ccteams added to that skill (e.g. a new script) without re-applying a team.
  3. It **reports** an existing `.claude/skills/team-lessons/SKILL.md` whose catalog index is in an older layout — either lacking a usable marker pair (absent, or the end marker first), or with the generated note still between the markers. That file is yours: ccteams prints the command to run (`gen-lessons.mjs`, plus the markers to add first when they are absent) and never rewrites the file itself.
  4. It **reports** any lesson under `.claude/skills/team-lessons/lessons/` whose frontmatter has no usable `applies_when` — `lessons/**` is read for this, but never written: ccteams cannot write the value itself (it would have to summarize the lesson's content, which is exactly the judgment call `applies_when` exists to record), so it lists the affected file(s) and points you at the scaffolded `AUTHORING.md` for the frontmatter schema and at asking Claude Code to fill each one in, followed by the regenerate command (`gen-lessons.mjs`). A lesson whose frontmatter can't even be parsed (no leading `---` block, or unreadable) is reported separately, since that has to be fixed by hand before the index can be regenerated at all — neither case ever aborts the rest of `ccteams migrate`.
  5. It **reports** whether the catalog-injection hook (see [The team-lessons skill](#the-team-lessons-skill) above, "Injecting the catalog via a hook") is registered in `.claude/settings.json` for `SessionStart` and `SubagentStart` — checked and reported individually, since registering only one still leaves a gap. Hooks run an arbitrary command on every session, so ccteams never writes this registration for you: when something is missing, it prints a copy-pasteable JSON fragment for the missing event(s) instead. Once both are registered, `ccteams migrate` reports nothing further about them.
  6. It **reconciles ccteams-owned files** — agent definitions (`.claude/agents/*.md`), every team's playbook skill (`.claude/skills/<team>-playbook/`), and the shared `.claude/skills/working-method/` skill — against what the currently installed ccteams package would place there today. Each file is classified by comparing three things: the package's current source, the project's current content, and the hash ccteams recorded as the baseline when it placed the file:
     ...
     A file the manifest still lists but the package can no longer find a source for (a renamed/removed team, for instance) is left untouched and reported, never deleted. Nothing under `.claude/skills/team-lessons/` is part of *this* step — that skill is never recorded in the manifest, so it is handled entirely by step 1 above, under its own rules.
- **`--yes` vs. `--force`:** `--yes` alone skips the confirmation prompt but still only auto-applies files ccteams has verified you never touched (an upstream-changed file, or a team-lessons template file whose content matches a version ccteams shipped) — it deliberately never overwrites a user-modified, unknown-baseline, or unverifiable template file, so an unattended CI run can never silently discard your edits. `--force` only takes effect together with `--yes` (running it alone is rejected with an error) and is what additionally authorizes overwriting those files too.
- **Non-interactive environments (CI, piped stdin) are never prompted.** Without `--yes`, a file needing a decision is simply left alone and reported — `ccteams migrate` never blocks waiting for input it cannot get.
- **Nothing is ever deleted.** Within `.claude/skills/team-lessons/`, your lessons — `SKILL.md` and everything under `lessons/` — are never rewritten, overwritten, or deleted by `ccteams migrate` under any flag combination. The tooling ccteams ships beside them (`AUTHORING.md`, `scripts/**`) *can* be updated, on the terms in step 1 above: silently only when ccteams can verify your copy is an untouched version it shipped, and otherwise only with `--yes --force` or your answer at the prompt.
- **`--dry-run` writes nothing** and lists what would be added/updated. It exits `1` when ccteams would add or update a file WITHOUT asking (a missing file, an upstream-changed file, or a team-lessons template file whose content ccteams verified as an untouched version it shipped) and `0` otherwise (so it composes with CI drift-checks the same way `gen-lessons.mjs --check` does — see above). **Notices, and files skipped pending your decision, do not affect the exit code:** a report that ends in a note about your `SKILL.md`, or about a user-modified file it left alone, still exits `0` — read the summary line, not just the exit code.
