...
- **Manifest v4 and older ccteams versions:** the reconciliation above needs a per-file baseline hash, so the manifest format moved to v4 (`.claude/.ccteams-manifest.json`'s `"version": "4"`). This is written automatically the next time you run `ccteams use` or `ccteams migrate`. A project whose manifest is still v3 or earlier works fine with the current ccteams (it degrades to the "unknown baseline" case above until its baseline hashes are recorded), but the reverse does not hold: an OLDER ccteams CLI that only understands up to v3 cannot read a v4 manifest, treats the project as if no team were applied, and then aborts on `ccteams use` because the collision guard sees every ccteams-placed file as unexpectedly "hand-written". If your team pins a ccteams version, upgrade everyone together rather than leaving some machines on a pre-v4 CLI against a v4-manifest project.
