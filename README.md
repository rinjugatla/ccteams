...
- **Keep one ccteams version per machine:** don't switch back and forth between an old and a new `ccteams` binary — install it globally once, so a project's manifest and the CLI reading it never disagree. As of this writing, the `ccteams` package published to npm predates v4 and cannot read it; a v4-aware CLI comes from this repository's source (`npm install -g .`, see [Development / local testing](#development--local-testing)). If you hit the abort described above, the CLI is what to upgrade — the manifest is never downgraded to suit an older CLI.
