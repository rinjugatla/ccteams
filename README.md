...
- **What it does today:** five things, layered so nothing that could lose your work happens without your say-so.
  1. It **updates the ccteams-owned files inside `.claude/skills/team-lessons/`** — `AUTHORING.md` and `scripts/**` (`gen-lessons.mjs`, `lessons-index.mjs`, `template-version.mjs`). Your `SKILL.md` and `lessons/**` are **never** read, written, or reported by this: an older ccteams stored every lesson in `SKILL.md` itself, so overwriting it would destroy exactly the knowledge the skill exists to keep. How the decision is made:
